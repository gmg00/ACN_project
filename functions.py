...
def plot_user_positions(df, edges, date, rounded_time, map_image_path):
    """
    Plots user positions and edges between them for a specific date and time on a map.

    Parameters:
        df (pd.DataFrame): DataFrame containing user IDs, positions, and times.
        edges (dict): Dictionary of edges for each rounded time, where keys are times and values are lists of user connections.
        date (datetime.date): Date to filter the DataFrame.
        rounded_time (str): Specific rounded time (e.g., "16:38") to plot.
    ...
    df_filtered = df[(df['date'] == date) & (df['rounded_time'] == rounded_time)]
    ...
    ax.imshow(map_image, extent=[-2400, 2400, -2400, 2400])  # Adjust extent to your map's coordinates

    # Plot edges
    for edge in edges.get(rounded_time, []):
        user1_pos = df_filtered[df_filtered['id'] == edge[0]]['position'].tolist()
        user2_pos = df_filtered[df_filtered['id'] == edge[1]]['position'].tolist()

        if user1_pos and user2_pos:  # Ensure positions exist
            position1 = [user1_pos[0][0], user1_pos[0][2]]  # Extract X, Z coordinates
            position2 = [user2_pos[0][0], user2_pos[0][2]]  # Extract X, Z coordinates
            ax.plot([position1[0], position2[0]], [position1[1], position2[1]], marker='o', color='red', markeredgecolor='k')
            ax.plot([position1[0], position2[0]], [position1[1], position2[1]], linewidth=0.5, color='black')

    # Set labels and title
    ax.set_xlabel('X Coordinate')
    ax.set_ylabel('Y Coordinate')
    ax.set_title(f'User Movements on {date} at {rounded_time}')
